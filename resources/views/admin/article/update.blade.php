...
@section('content')
    ...
    <div class="container-fluid">
        ...
        <!-- Content Row -->
        <form class="row" action="{{ route('update.article', $article_detail) }}" method="post"
                ...
                <div class="form-group">
                    ...
                    <select name="category" class="form-control">
                        ...
                        @foreach ($categories as $row)
                            <option value="{{ $row->id }}"
                                {{ $row->id == ($article_detail->category->id ?? -1) ? 'selected' : '' }}>
                                {{ $row->name }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="form-group">
                    ...
                    <select name="journalist" class="form-control">
                        @foreach ($journalists as $row)
                            <option value="{{ $row->id }}"
                                {{ $row->id == $article_detail->journalist->id ? 'selected' : '' }}>{{ $row->name }}
                            </option>
                        @endforeach
                    </select>
                </div>
            ...
            <div class="col-12">
                <div class="form-group">
                    ...
                    <input type="text" name="image-caption" class="form-control" placeholder="Caption" required
                        value="{{ $article_detail->image_caption }}">
                </div>
            </div>
            <div class="col-12">
                <div class="form-group">
                    <label>Isi Konten Berita</label>
                    <input id="x" type="hidden" name="content" value="{{ $article_detail->content }}" />
