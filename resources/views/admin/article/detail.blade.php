...
@section('content')
    ...
    <div class="container-fluid">
        ...
        <div class="d-sm-flex align-items-center justify-content-between mb-4">
            <div>
                <a href="{{ route('admin.article.update', $article_detail->id) }}"
                ...
                <form class="d-sm-inline-block" action="{{ route('delete.article', $article_detail) }}" method="post">
                    ...
                    <button class="btn btn-sm btn-danger shadow-sm"
                        onclick="return confirm('Apakah anda yakin ingin menghapus artikel ini?');"><i
                            class="fas fa-trash fa-sm text-white-50"></i> Hapus Artikel</buton>
                </form>
            ...
            <ol class="breadcrumb">
                ...
                @if ($article_detail->category)
                    <li class="breadcrumb-item"><a
                            href="{{ route('admin.category.detail', $article_detail->category) }}">{{ $article_detail->category->name }}</a>
                    </li>
            ...
            <div class="col-12">
                <div class="article-header">
                    ...
                    <div class="article-meta">
                        ...
                        <span class="meta-item"><i class="fa fa-clock-o" aria-hidden="true"></i>
                            {{ $article_detail->formated_created_date }} (
                            {{ $article_detail->created_at->diffForHumans() }})</span>
                        <span class="meta-item"><i class="fa fa-eye" aria-hidden="true"></i> Dikunjungi
                            {{ count($article_detail->page_views) }} kali</span>
                    </div>
                </div>
                ...
                <div class="article-content">
                    <div class="article-image">
                        <img src="{{ asset('img/article/' . $article_detail->image_url) }}"
                            alt="{{ $article_detail->image_caption }}">
                        <p style="text-align: end;">{{ $article_detail->image_caption }}</p>
                    </div>
                    <div style="text-align: justify">
                        @php
                            echo $article_detail->content;
                        @endphp
                        <br>
                    </div>
